   ...
   if (!initialized) {
	...
	for (i = 0; i < 8 * nb_MCU_V; i++) {
		for (j = 0; j < 8 * nb_MCU_H; j += 4) {
          ...
          * This is powerfull (but hard to understand, poor compiler) */
...
#endif
         /* Cr Cb Computation */
         uint32_t x = 0x80808080;
         __asm__("punpcklbw %0,  %%mm7" :: "m"(x));
         __asm__("psrlq     $8, %mm7");
         ...
         /* R is in mm3, both words contain the same thing! */
         __asm__("psrlw     $8, %mm3");
         ...
         /* Both words contain the same thing! */
         __asm__("psrlw     $8, %mm4");
         ...
         /* B is in mm5, both words contain the same thing! */
         __asm__("psrlw     $8, %mm5");
